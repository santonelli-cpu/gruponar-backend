...
function streamToResponse(key, res, { contentType, downloadName, inline } = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (downloadName) {
      // Los nombres reales traen acentos/ñ ("Constancia de Situación.pdf") —
      // un header solo acepta ASCII, así que va el par estándar RFC 5987:
      // filename= con fallback ASCII y filename*= con el nombre real UTF-8.
      // Antes se mandaba el nombre crudo y setHeader tronaba (o el nombre
      // llegaba corrupto) con cualquier documento nombrado en español.
      const ascii = downloadName.replace(/["\\\r\n]/g, '').replace(/[^\x20-\x7e]/g, '_');
      const utf8 = encodeURIComponent(downloadName.replace(/[\r\n]/g, ''));
      res.setHeader('Content-Disposition', `${inline ? 'inline' : 'attachment'}; filename="${ascii}"; filename*=UTF-8''${utf8}`);
    }
...
// Duplica un objeto dentro del mismo bucket — se usa al traer los
// documentos de un cliente de una operación anterior a una nueva. Se COPIA
// a propósito en vez de apuntar las dos operaciones al mismo objeto:
// borrar una operación borra todo su prefijo (deletePrefix), y con un
// objeto compartido eso dejaría a la otra sin su archivo.
...
// Nombres de los objetos bajo un prefijo — lo usa la poda de respaldos
// (lib/dbBackup.js) para saber qué días ya están respaldados.
...
module.exports = {
  ...
  existsFile, streamToResponse, copyFile, deleteFile, deletePrefix, listFiles
};

...
// Resumen de avance del tracker de cierre (POST /api/deals/:id/
// send-progress-summary): "se completaron los pasos 1, 2, 3 — sigue el 4".
// Lo dispara admin/abogado interno cuando quiere mantener a las partes al
// tanto sin que tengan que entrar al portal a revisar.
function sendProgressSummaryEmail({ to, name, dealProperty, completedSteps, nextStep, url, lang }) {
  const isEn = lang === 'en';
  const list = completedSteps.map(s => `<li style="margin-bottom:4px;">${escapeHtml(s)}</li>`).join('');
  const completedBlock = completedSteps.length
    ? (isEn
      ? `These steps of the closing are now complete:<br><br><ul style="padding-left:20px; margin:0 0 12px;">${list}</ul>`
      : `Estos pasos del cierre ya están completados:<br><br><ul style="padding-left:20px; margin:0 0 12px;">${list}</ul>`)
    : (isEn ? 'The closing process is underway.<br><br>' : 'El proceso de cierre está en marcha.<br><br>');
  const nextBlock = nextStep
    ? (isEn ? `<b>Next step:</b> ${escapeHtml(nextStep)}.` : `<b>Siguiente paso:</b> ${escapeHtml(nextStep)}.`)
    : (isEn ? '<b>All tracker steps are complete.</b>' : '<b>Todos los pasos del tracker están completados.</b>');
  const bodyHtml = (isEn ? `Hi ${escapeHtml(name)},<br><br>` : `Hola ${escapeHtml(name)},<br><br>`)
    + (isEn ? `Progress update on the closing of <b>${escapeHtml(dealProperty)}</b>:<br><br>` : `Avance del cierre de <b>${escapeHtml(dealProperty)}</b>:<br><br>`)
    + completedBlock + nextBlock;
  return sendEmail({
    to,
    subject: isEn ? `Closing progress — ${dealProperty}` : `Avance de tu cierre — ${dealProperty}`,
    html: renderShell({ title: isEn ? 'Closing progress' : 'Avance de tu cierre', bodyHtml, ctaLabel: isEn ? 'Go to the portal' : 'Ir al portal', ctaUrl: url })
  });
}

// Aviso al abogado interno/admin al que el admin le asigna una tarea del
// tracker (PATCH /api/deals/:id/tasks/:taskId con assignedTo) — para que se
// entere sin tener que entrar a revisar la operación.
function sendTaskAssignedEmail({ to, name, dealProperty, taskLabel, assignedByName, url }) {
  const bodyHtml = `Hola ${escapeHtml(name)},<br><br>`
    + `${escapeHtml(assignedByName)} te asignó una tarea en la operación <b>${escapeHtml(dealProperty)}</b>:<br><br>`
    + `<b>${escapeHtml(taskLabel)}</b>`;
  return sendEmail({
    to,
    subject: `Tarea asignada — ${dealProperty}`,
    html: renderShell({ title: 'Te asignaron una tarea', bodyHtml, ctaLabel: 'Ver la operación', ctaUrl: url })
  });
}

// Un agente (o abogado externo) llenó y generó un documento que NO puede
// mandar a firma él mismo: queda esperando que admin/abogado interno lo
// revise y lo autorice. Sin este aviso, el documento se quedaba esperando
// hasta que alguien entrara a la operación por su cuenta y lo notara.
function sendAuthorizationNeededEmail({ to, name, dealProperty, documentLabel, requestedByName, url }) {
  const bodyHtml = `Hola ${escapeHtml(name)},<br><br>`
    + `${escapeHtml(requestedByName)} preparó un documento en la operación <b>${escapeHtml(dealProperty)}</b> `
    + `y está esperando tu autorización para mandarse a firma:<br><br>`
    + `<b>${escapeHtml(documentLabel)}</b><br><br>`
    + `Revísalo y, si está bien, mándalo a firma desde la operación.`;
  return sendEmail({
    to,
    subject: `Pendiente de autorizar — ${dealProperty}`,
    html: renderShell({ title: 'Un documento espera tu autorización', bodyHtml, ctaLabel: 'Revisar y autorizar', ctaUrl: url })
  });
}

// Recordatorio interno de fecha límite (lib/deadlineReminders.js): la fecha
// de cierre o el fin de due diligence de una operación está a N días — va
// al equipo (admin/abogados de la operación), no a los clientes.
function sendDeadlineReminderEmail({ to, name, dealProperty, kindLabel, targetDate, daysLeft, url }) {
  const bodyHtml = `Hola ${escapeHtml(name)},<br><br>`
    + `La operación <b>${escapeHtml(dealProperty)}</b> tiene <b>${escapeHtml(kindLabel)}</b> el <b>${escapeHtml(targetDate)}</b> — `
    + (daysLeft <= 2 ? `en <b>${daysLeft} día${daysLeft === 1 ? '' : 's'}</b>.` : `en ${daysLeft} días.`)
    + `<br><br>Revisa que los documentos, firmas y pasos del tracker vayan a tiempo.`;
  return sendEmail({
    to,
    subject: `⏰ ${kindLabel} en ${daysLeft} día${daysLeft === 1 ? '' : 's'} — ${dealProperty}`,
    html: renderShell({ title: 'Fecha límite próxima', bodyHtml, ctaLabel: 'Ver la operación', ctaUrl: url })
  });
}
...
// Se manda al agente/abogado externo cuando el admin aprueba su cuenta
// autoregistrada (PATCH /api/users/:id/approve) — antes de esto no había
// ningún aviso, la persona tenía que adivinar o volver a intentar entrar
// por su cuenta para enterarse de que ya podía usar el portal.
function sendAccountApprovedEmail({ to, name, url }) {
  const bodyHtml = `Hola ${escapeHtml(name)},<br><br>Tu cuenta en el portal de Grupo Nar ya fue aprobada — ya puedes iniciar sesión con tu correo y contraseña.`;
  return sendEmail({
    to, subject: 'Tu acceso al portal de Grupo Nar ya está disponible',
    html: renderShell({ title: 'Tu cuenta ya está activa', bodyHtml, ctaLabel: 'Iniciar sesión', ctaUrl: url })
  });
}

// Segundo factor por correo — alternativa a la app de autenticación (ver
// routes/auth.js POST /auth/email-otp). Sin CTA (no hay a dónde dar clic,
// el código se copia a mano), solo el código grande y cuánto dura.
function sendTwoFactorCodeEmail({ to, name, code, lang }) {
  const isEn = lang === 'en';
  const codeHtml = `<div style="text-align:center; margin:18px 0; font-family:'Inter', Arial, sans-serif; font-size:32px; font-weight:700; letter-spacing:8px; color:#3A0C08;">${escapeHtml(code)}</div>`;
  const bodyHtml = isEn
    ? `Hi ${escapeHtml(name)},<br><br>Here's your verification code to log in to the Grupo Nar portal:${codeHtml}This code expires in 10 minutes. If you didn't request it, you can ignore this email.`
    : `Hola ${escapeHtml(name)},<br><br>Este es tu código de verificación para entrar al portal de Grupo Nar:${codeHtml}Este código vence en 10 minutos. Si tú no lo pediste, puedes ignorar este correo.`;
  return sendEmail({
    to, subject: isEn ? 'Your verification code' : 'Tu código de verificación',
    html: renderShell({ title: isEn ? 'Verification code' : 'Código de verificación', bodyHtml })
  });
}
...
module.exports = { isConfigured, sendEmail, sendInviteEmail, sendDocumentReminderEmail, sendStatusUpdateEmail, sendPasswordResetEmail, sendAgentAddedToDealEmail, sendPendingApprovalEmail, sendAccountApprovedEmail, sendPredialReminderEmail, sendTwoFactorCodeEmail, sendProgressSummaryEmail, sendDeadlineReminderEmail, sendTaskAssignedEmail, sendAuthorizationNeededEmail, resolveClientLang };

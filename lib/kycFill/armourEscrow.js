...
function fillArmourEscrow(deal, fields, outputDocxPath, sellerNames, buyerNames) {
  ...
  try {
    ...
    // casillas y del bloque de firmas de la plantilla) — nunca se llenaba
    // porque el fill original nunca llegaba hasta ahí. El primer blanco en
    // inglés ("...purchase agreement (____) described in Box 4...") es la
    // fecha del contrato de compraventa referenciado — no hay un campo
    // propio para eso todavía, se deja en blanco a propósito en vez de
    // adivinar. La versión en español no tiene ese primer blanco.

...
function createRateLimiter({ windowMs, maxAttempts, keyBy = 'ip' }) {
  ...
  return function rateLimit(req, res, next) {
    // Solo para la suite de tests (test/smoke.test.js): tantos logins
    // seguidos desde 127.0.0.1 disparan el límite real. Nunca se define en
    // producción — es un opt-in explícito del proceso de test.
    if (process.env.DISABLE_RATE_LIMITS === '1') return next();
    const now = Date.now();

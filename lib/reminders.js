...
async function runAutomaticReminders() {
  ...
  const linkedParties = db.prepare(`
    SELECT dpe.id AS partyId, dpe.side, dpe.deal_id AS dealId, d.property AS dealProperty, d.scenario AS scenario, u.name AS userName, u.email AS userEmail
  ...
  for (const p of linkedParties) {
    ...
    const result = await mailer.sendDocumentReminderEmail({
      ...
      lang: mailer.resolveClientLang(p.scenario, p.side)
    });
    if (result.ok) {
      db.prepare(`

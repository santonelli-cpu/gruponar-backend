...
// el body).
router.patch('/:id', requireRole('admin', 'lawyer', 'external_lawyer'), rateLimitWrite, validateBody(updateDealSchema), (req, res) => {
...
router.post('/:id/parties', requireRole('admin', 'lawyer', 'external_lawyer'), rateLimitWrite, validateBody(createPartySchema), async (req, res) => {
  if (!canAccessDeal(req, req.params.id)) return res.status(403).json({ error: 'No autorizado.' });
...
router.patch('/:id/parties/:partyId', requireRole('admin', 'lawyer', 'external_lawyer'), rateLimitWrite, validateBody(updatePartySchema), async (req, res) => {
  if (!canAccessDeal(req, req.params.id)) return res.status(403).json({ error: 'No autorizado.' });
...
router.post('/:id/parties/:partyId/rebuild-checklist', requireRole('admin', 'lawyer', 'external_lawyer'), rateLimitWrite, (req, res) => {
  if (!canAccessDeal(req, req.params.id)) return res.status(403).json({ error: 'No autorizado.' });
  const party = db.prepare('SELECT * FROM deal_party_entities WHERE id = ? AND deal_id = ?').get(req.params.partyId, req.params.id);
  if (!party) return res.status(404).json({ error: 'Parte no encontrada.' });
  const deal = db.prepare('SELECT * FROM deals WHERE id = ?').get(req.params.id);
...
router.post('/:id/parties/:partyId/fix-entity-checklist', requireRole('admin', 'lawyer', 'external_lawyer'), rateLimitWrite, (req, res) => {
  if (!canAccessDeal(req, req.params.id)) return res.status(403).json({ error: 'No autorizado.' });
  const party = db.prepare('SELECT * FROM deal_party_entities WHERE id = ? AND deal_id = ?').get(req.params.partyId, req.params.id);
  if (!party) return res.status(404).json({ error: 'Parte no encontrada.' });
...
router.delete('/:id/parties/:partyId', requireRole('admin', 'lawyer', 'external_lawyer'), rateLimitWrite, (req, res) => {
  if (!canAccessDeal(req, req.params.id)) return res.status(403).json({ error: 'No autorizado.' });
  const party = db.prepare('SELECT * FROM deal_party_entities WHERE id = ? AND deal_id = ?').get(req.params.partyId, req.params.id);
  if (!party) return res.status(404).json({ error: 'Parte no encontrada.' });
...
router.post('/:id/parties/from-client', requireRole('admin', 'lawyer', 'external_lawyer'), rateLimitWrite, validateBody(fromClientSchema), async (req, res) => {
  if (!canAccessDeal(req, req.params.id)) return res.status(403).json({ error: 'No autorizado.' });
  const deal = db.prepare('SELECT * FROM deals WHERE id = ? AND deleted_at IS NULL').get(req.params.id);
  if (!deal) return res.status(404).json({ error: 'Operación no encontrada.' });
...
router.post('/:id/agents', requireRole('admin', 'lawyer', 'external_lawyer'), rateLimitWrite, validateBody(addAgentSchema), (req, res) => {
  if (!canAccessDeal(req, req.params.id)) return res.status(403).json({ error: 'No autorizado.' });
...
router.post('/:id/agents/register', requireRole('admin', 'lawyer', 'external_lawyer'), rateLimitEmail, validateBody(registerAgentSchema), async (req, res) => {
  if (!canAccessDeal(req, req.params.id)) return res.status(403).json({ error: 'No autorizado.' });
...
router.patch('/:id/agents/:userId', requireRole('admin', 'lawyer', 'external_lawyer'), rateLimitWrite, validateBody(updateAgentSchema), (req, res) => {
  if (!canAccessDeal(req, req.params.id)) return res.status(403).json({ error: 'No autorizado.' });
...
router.delete('/:id/agents/:userId', requireRole('admin', 'lawyer', 'external_lawyer'), rateLimitWrite, (req, res) => {
  if (!canAccessDeal(req, req.params.id)) return res.status(403).json({ error: 'No autorizado.' });
...
router.post('/:id/drive-folder', requireRole('admin', 'lawyer', 'external_lawyer'), rateLimitWrite, async (req, res) => {
  if (!canAccessDeal(req, req.params.id)) return res.status(403).json({ error: 'No autorizado.' });
...
router.post('/:id/documents', requireRole('admin', 'lawyer', 'external_lawyer'), rateLimitWrite, validateBody(addDocumentSchema), (req, res) => {
  if (!canAccessDeal(req, req.params.id)) return res.status(403).json({ error: 'No autorizado.' });
...
router.delete('/:id/documents/:docId', requireRole('admin', 'lawyer', 'external_lawyer'), rateLimitWrite, async (req, res) => {
  if (!canAccessDeal(req, req.params.id)) return res.status(403).json({ error: 'No autorizado.' });
...
router.delete('/:id', requireRole('admin', 'lawyer', 'external_lawyer'), rateLimitWrite, (req, res) => {
  if (!canAccessDeal(req, req.params.id)) return res.status(403).json({ error: 'No autorizado.' });
